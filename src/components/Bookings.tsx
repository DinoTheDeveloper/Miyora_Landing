...
import diningTable from "../assets/images/golden-hour-dinner-table.webp";
...
function Bookings() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const fullName = sanitizeLine(
      String(data.get("full_name") ?? ""),
      MAX_NAME_LENGTH,
    );
    const email = sanitizeLine(
      String(data.get("email") ?? ""),
      MAX_NAME_LENGTH,
    );
    const eventType = sanitizeLine(
      String(data.get("event_type") ?? ""),
      MAX_NAME_LENGTH,
    );
    const preferredDate = sanitizeLine(
      String(data.get("preferred_date") ?? ""),
      20,
    );
  ...
  return (
    <section id="bookings" data-nav-section className="bookings">
      ...
      <p className="bookings__subtext">
        Tell us a little about what you are planning, and we will be in touch
        to arrange a time to talk it through.
      </p>
        ...
        <form className="bookings__form" onSubmit={handleSubmit} noValidate>
          ...
          <div className="bookings__field" ref={eventFieldRef}>
            ...
            <div className="bookings__select">
              ...
              >
                <span
                  className={eventType ? "" : "bookings__select-placeholder"}
                >
          ...
          {status === "error" && (
            <p className="bookings__status bookings__status--error">
              Something went wrong sending your enquiry. Please check your
              details and try again, or email us directly at
              info@miyoracollective.co.za.
            </p>
          )}
          {status === "blocked" && (
            <p className="bookings__status bookings__status--error">
              You've already sent an enquiry recently — please wait a moment
              before sending another.
            </p>
          )}
        </form>

        <div className="bookings__accent">
          <div className="bookings__image">
            <img
              src={diningTable}
              alt="Golden hour dinner table styled for an event"
              width={1176}
              height={1302}
              loading="lazy"
            />
